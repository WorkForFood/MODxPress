...
if(!include_once(MODX_MANAGER_PATH.'media/style/'.$modx->config['manager_theme'].'/includes/render.class.inc.php')) { die ("can't include renderer"); }
			...
			if ($replace =='') {
				...
				if (!$isfolder) {
					...
					if ($expandAll ==1 || ($expandAll == 2 && in_array($id, $opened)))
					{
						...
						$renderer->setPlaceholders(Array(
							'ph.id' => $id, 'ph.pagetitle' => $pagetitle, 'ph.menutitle' => $menutitle,'ph.parent' =>$parent,
							'ph.isfolder' =>$isfolder,'ph.published' =>$published,'ph.deleted' =>$deleted,'ph.type' =>$type,'ph.menuindex' =>$menuindex,
							'ph.donthit' =>$donthit,'ph.hidemenu' =>$hidemenu,'ph.alias' =>$alias,'ph.contenttype' =>$contenttype,'ph.privateweb' =>$privateweb,
							'ph.privatemgr' =>$privatemgr,'ph.hasAccess' => $hasAccess, 'ph.template' => $template,
							'ph.nodetitle' => $nodetitle, 'ph.spacer' => $spacer, 'ph.pad' => $pad, 'ph.url' => $url, 'ph.alt' => $alt,
							'ph.nodetitleDisplay' => $nodetitleDisplay,'ph.weblinkDisplay' => $weblinkDisplay,'ph.pageIdDisplay' => $pageIdDisplay,
							"tree_folderopen" => $tree_folderopen, "ph.nodetitle"=>addslashes($nodetitle), "ph.tree_minusnode"=>$_style["tree_minusnode"], "ph.indentplus" => $indent+1, "ph.privatewebmgr" => $privatewebmgr, 
							"ph.expandAll" => $expandAll, "ph.childnodes" => $childnodes, "ph.display" => "block"
						));
						...
						$renderer->setPlaceholders(Array(
							"tree_folderopen" => $tree_folderopen, "ph.nodetitle"=>addslashes($nodetitle), "ph.tree_minusnode"=>$_style["tree_plusnode"], "ph.indentplus" => $indent+1, "ph.privatewebmgr" => $privatewebmgr, 
							"ph.expandAll" => $expandAll, "ph.display" => "none"
						));
