...
<?php if($_POST['mode']=='save'||$_GET['mode']=='edit') :?>
	<li><a href="#" onclick="document.editFile.submit();"><span class="<?php echo $_style["icons_save"] ?>"></span> <?php echo $_lang['save']?></a></li>
<?php endif; ?>
<?php
...
if (is_writable($startpath))
{
	...
	$tpl = '<li><a href="[+href+]" onclick="return getFolderName(this);"><span class="[+image+]"></span> [+subject+]</a></li>';
	$ph['image']   = $_style['icons_folder'];
	$ph['subject'] = $_lang['add_folder'];
	$ph['href'] = 'index.php?a=31&mode=newfolder&path='.urlencode($startpath).'&name=';
	$_ = parsePlaceholder($tpl,$ph);
	
	$tpl = '<li><a href="[+href+]" onclick="return getFileName(this);"><span class="[+image+]"></span> ' . $_lang['files.dynamic.php1'] . '</a></li>';
	$ph['image']   = $_style['icons_page_php'];
	$ph['href'] = 'index.php?a=31&mode=newfile&path='.urlencode($startpath).'&name=';
	$_ .=  parsePlaceholder($tpl,$ph);
	echo $_;
}
?>
      <li id="Button5"><a href="#" onclick="documentDirty=false;document.location.href='index.php?<?php echo $href;?>';"><span class="<?php echo $_style["icons_cancel"] ?>"></span> <?php echo $_lang['cancel']?></a></li>
  </ul>
</div>
...
if($folders==0 && $files==0) {
	echo '<tr><td colspan="4"><span style="color:#888;cursor:default;"> This directory is empty.</span></td></tr>';
}
?></table>
<hr />
<?php
...
<a class="default" href="#" onclick="document.upload.submit()" style="display:inline;float:none;"><?php echo '<span class="fileactions ' . $_style['icons_add'] . '"></span> '; echo $_lang['files_uploadfile'];?></a>
...
function ls($curpath)
{
	...
	foreach ($dir as $file)
	{
		...
		if(is_dir($newpath))
		{
			...
			{
				$dirs_array[$dircounter]['text'] = '<span class="fileactions '.$_style['icons_folder'].'" align="absmiddle"></span> <a href="index.php?a=31&mode=drill&path='.urlencode($newpath).'"><b>'.$file.'</b></a>';
				...
				$dirs_array[$dircounter]['delete'] = is_writable($curpath) ? '<span style="width:20px"><a href="javascript: deleteFolder(\''.urlencode($file).'\',\'' . $file_exists . '\');"><span class="fileactions '.$_style['icons_delete'].'" alt="'.$_lang['file_delete_folder'].'" title="'.$_lang['file_delete_folder'].'"></span></a></span>' : '';
			}
			else
			{
				$dirs_array[$dircounter]['text'] = '<span class="fileactions '.$_style['icons_delete_folder'].'" align="absmiddle"></span> <span style="color:#bbb;">'.$file.'</span>';
				$dirs_array[$dircounter]['delete'] = is_writable($curpath) ? '<span style="width:20px" class="disabledImage"><span class="fileactions '.$_style['icons_delete'].'" alt="'.$_lang['file_delete_folder'].'" title="'.$_lang['file_delete_folder'].'"></span></span>' : '';
			}
		...
		else
		{
			...
			$files_array[$filecounter]['text'] = '<span class="fileactions '.$_style['icons_page_html'].'" align="absmiddle"></span> '.$file;
			$files_array[$filecounter]['view'] = (in_array($type, $viewablefiles)) ?
			'<span style="cursor:pointer; width:20px;" onclick="viewfile(\''.$webstart_path.substr($newpath, $len, strlen($newpath)).'\');"><span class="fileactions '.$_style['icons_show'].'" align="absmiddle" alt="'.$_lang['files_viewfile'].'" title="'.$_lang['files_viewfile'].'"></span></span> ' : (($enablefiledownload && in_array($type, $uploadablefiles))? '<a href="'.$webstart_path.implode('/', array_map('rawurlencode', explode('/', substr($newpath, $len, strlen($newpath))))).'" style="cursor:pointer; width:20px;"><span class="fileactions '.$_style['icons_save'].'" align="absmiddle" alt="'.$_lang['file_download_file'].'" title="'.$_lang['file_download_file'].'"></span></a> ':'<span class="disabledImage"><span class="fileactions '.$_style['icons_show'].'" align="absmiddle" alt="'.$_lang['files_viewfile'].'" title="'.$_lang['files_viewfile'].'"></span></span> ');
			$files_array[$filecounter]['view'] = (in_array($type, $inlineviewablefiles)) ? '<span style="width:20px;"><a href="index.php?a=31&mode=view&path='.urlencode($newpath).'"><span class="fileactions '.$_style['icons_show'].'" align="absmiddle" alt="'.$_lang['files_viewfile'].'" title="'.$_lang['files_viewfile'].'"></span></a></span> ' : $files_array[$filecounter]['view'] ;
			$files_array[$filecounter]['unzip'] = ($enablefileunzip && $type=='.zip') ? '<span style="width:20px;"><a href="javascript:unzipFile(\''.urlencode($file).'\');"><span class="fileactions '.$_style['icons_unzip'].'" align="absmiddle" alt="'.$_lang['file_download_unzip'].'" title="'.$_lang['file_download_unzip'].'"></span></a></span> ' : '' ;
			$files_array[$filecounter]['edit'] = (in_array($type, $editablefiles) && is_writable($curpath) && is_writable($newpath)) ? '<span style="width:20px;"><a href="index.php?a=31&mode=edit&path='.urlencode($newpath).'#file_editfile"><span class="fileactions '.$_style['icons_edit_document'] . '" align="absmiddle" alt="'.$_lang['files_editfile'].'" title="'.$_lang['files_editfile'].'"></span></a></span> ' : '<span class="disabledImage"><span class="fileactions '.$_style['icons_edit_document'].'" align="absmiddle" alt="'.$_lang['files_editfile'].'" title="'.$_lang['files_editfile'].'"></span></span> ';
            $files_array[$filecounter]['delete'] = is_writable($curpath) && is_writable($newpath) ? '<span style="width:20px;"><a href="javascript:deleteFile(\''.urlencode($file).'\');"><span class="fileactions '.$_style['icons_delete'].'" align="absmiddle" alt="'.$_lang['file_delete_file'].'" title="'.$_lang['file_delete_file'].'"></span></a></span> ' : '<span class="disabledImage"><span class="fileactions '.$_style['icons_delete'].'" align="absmiddle" alt="'.$_lang['file_delete_file'].'" title="'.$_lang['file_delete_file'].'"></span></span> ';
